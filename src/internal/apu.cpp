...
NoiseChannel::NoiseChannel(Envelope const& envelope) :
    Channel(TU::NOISE_DEFAULT_PERIOD),
    mEnvelope(envelope),
    mValidScf(true),
    mHalfWidth(false),
    mLfsr(TU::LFSR_INIT)
...
void NoiseChannel::reset() noexcept {
    ...
    timer().setPeriod(TU::NOISE_DEFAULT_PERIOD);
}
...
PulseChannel::PulseChannel(Envelope const& envelope) :
    Channel(TU::PULSE_DEFAULT_PERIOD),
    mEnvelope(envelope),
    mDuty(Duty75),
...
void PulseChannel::setFrequency(uint16_t freq) noexcept {
    mFrequency = freq;
    timer().setPeriod((2048 - freq) * TU::PULSE_MULTIPLIER);
}
...
void PulseChannel::reset() noexcept {
    timer().setPeriod(TU::PULSE_DEFAULT_PERIOD);
    mDutyCounter = 0;
    setDuty(Duty75);
    Channel::reset();
}
...
WaveChannel::WaveChannel() :
    Channel(TU::WAVE_DEFAULT_PERIOD),
...
void WaveChannel::setFrequency(uint16_t frequency) noexcept {
    ...
    timer().setPeriod((2048 - mFrequency) * TU::WAVE_MULTIPLIER);
}
...
void WaveChannel::reset() noexcept {
    timer().setPeriod(TU::WAVE_DEFAULT_PERIOD);
    mVolumeShift = 0;
    mWaveVolume = VolumeMute;
    mWaveram.fill((uint8_t)0);
    mSampleBuffer = 0;
...
Sequencer::Trigger const Sequencer::TRIGGER_SEQUENCE[] = {
    ...
    {1,     TU::CYCLES_PER_STEP * 2,    TriggerType::lc},

    {2,     TU::CYCLES_PER_STEP * 2,    TriggerType::lcSweep},

    {3,     TU::CYCLES_PER_STEP,        TriggerType::lc},

    // step 6 trigger, next trigger: 7
    {4,     TU::CYCLES_PER_STEP,        TriggerType::lcSweep},

    // step 7 trigger, next trigger: 0
    {0,     TU::CYCLES_PER_STEP * 2,    TriggerType::env}
};

Sequencer::Sequencer() :
    mTimer(TU::SEQUENCER_DEFAULT_PERIOD),
    mTriggerIndex(0)
{
}

void Sequencer::reset() noexcept {
    mTimer.setPeriod(TU::SEQUENCER_DEFAULT_PERIOD);
    mTimer.restart();
    mTriggerIndex = 0;
}
...
Mixer::MixParam Mixer::getMixParameters(uint32_t cycletime) {
    ...
    float phase = (time - (int)time) * TU::PHASES;

    return {
        TU::STEP_TABLE[(int)(phase)],
...
void Mixer::setSamplerate(unsigned rate) {
    if (mSamplerate != rate) {
        mSamplerate = rate;
        mFactor = mSamplerate / GB_CLOCK_SPEED<float>;
