...
void ChannelControl<ch>::update(
    ...
    constexpr uint8_t REGS_START = IApuIo::REG_NR10 + (+ch * GB_CHANNEL_REGS);
    ...
    if (lastState.playing != state.playing) {
        if (state.playing) {
            ...
            // note cut
            // set panning to mute
            uint8_t nr51 = apu.readRegister(IApuIo::REG_NR51);
            nr51 &= ~(0x11 << +ch);
    ...
    if (writeEnvelope) {
        ...
        if constexpr (ch == ChType::ch3) {
            ...
            if (waveform != nullptr) {
                // DAC OFF
                apu.writeRegister(IApuIo::REG_NR30, 0x00);

                // copy wave
                auto &data = waveform->data();
                for (size_t i = 0; i != data.size(); ++i) {
                    apu.writeRegister((uint8_t)(IApuIo::REG_WAVERAM + i), data[i]);
                }

                // DAC ON
                apu.writeRegister(IApuIo::REG_NR30, 0x80);
                retrigger = true;
            }
    ...
    if constexpr (ch == ChType::ch4) {
        ...
        if (timbreChanged || freqChanged) {
            ...
            if (!freqChanged) {
                ...
                nr43 = apu.readRegister(IApuIo::REG_NR43);
                if (state.timbre) {
                    nr43 |= 0x08;
                } else {
                    nr43 &= ~0x08;
            ...
            apu.writeRegister(IApuIo::REG_NR43, nr43);
        }


        if (retrigger) {
            apu.writeRegister(IApuIo::REG_NR44, 0x80);
        }
    } else {
        ...
        if (timbreChanged) {
            if constexpr (ch == ChType::ch3) {
                ...
                apu.writeRegister(IApuIo::REG_NR32, vol);
